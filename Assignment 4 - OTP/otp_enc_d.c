...
*to encode a file using a key. The user must pass in a plaintext file, key, and correct
*port number
*******************************************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		case 0:
			...
			//if connection is encode-'e', then set permission to true, if not set to false
			if (handshake == 'e')
			{
				permission = 't';
			...
			//send permission status to otp_enc
			send(establishedConnectionFD, &permission, sizeof(char), 0);


			//if connection was valid, otp_enc will send size of both files, 
			...
			//string useed to store encryption
			char* encryption = encrypt(buffer, keybuffer, inputSize);
			//send encryption to otp_enc
			send(establishedConnectionFD, encryption, inputSize * sizeof(char), 0);
...
//functtion to encrypt input sent by otp_enc
//this function needs a string input, a key, and the inputs size
//in order to encrypt
char* encrypt(char* input, char* key, int size)
{
